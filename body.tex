...
To test these predictions, we assessed the behavior of the ABC-GLM
...
comparing the correct branch-length model (branch length ~ uniform(0.0001, 0.1)
to a model with a prior approximately twice as broad (branch length ~
uniform(0.0001, 0.2).

This very simple model is a nice test of the ABC-GLM for several reasons.
...
Also, quadrature integration using 1,000 and 10,000 steps and rectangular and
trapezoidal rules all yielded identical values for the log marginal likelihood
to at least five decimal places for all 100 simulated data sets,
providing a very precise proxy for the true values.
...
Given that the GLM approximation of the likelihood is only fit within regions
of parameter space with high likelihood, which is usually a \emph{very} small
region of a model, it never should have been considered a marginal likelihood
of the model.
...
\thought{We can't be comprehensive here. Subsections need to be
    integrated to discuss general themes of evolution we can learn
    about using marginal likelihoods}
...
Need to talk about how parameter estimation and model choice are different
beasts.
...
\thought{This is getting into the realm of Lindley's paradox; worth
    discussing?}
