...
\citet{Wang2018} introduced a variant of SMC that, similar to path sampling and
stepping stone, transitions from a sample from the prior distribution to the
posterior across a series of distributions where the likelihood is raised to
power (annealing).
Also, their approach maintains the full state space of the model while sampling
across these distributions, which allows them to use standard
...
the posterior distribution and estimaing the marginal likelihood, respectively.

\subsubsection{Nested sampling (NS)}
Recently, \citet{Maturana2017}
...
\citet{Baele2014} showed that model-averaging performed
...
\citet{Baele2012} demonstrated that modeling
...
Their use of marginal likelihoods allowed
\citet{Brown2017} to reveal a large degree of variation among loci in
