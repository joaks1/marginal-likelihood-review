...
The example above demonstrates that if we have informative data, this
objective Bayesian \citep{Jeffreys1961,Berger2006} strategy is defensible if
our goal is to infer the posterior distribution of a model;
...
marginal likelhoods among models.
...
One option is to take a more subjective Bayesian
\citep{Lad1996,Lindley2000,Goldstein2006} approach by carefully choosing prior
...
Alternatively, we can use hiearchical models that allow the data to inform the
...
spaced between the reference and posterior distribution, and how many samples
are needed from each?
...
via hiearchical modeling \citep{Suchard2003HPM}, the model with the largest PS
...
``uncontroversial'' biparitions when they calculate the marginal likelihoods of
...
``new'' data
(\data{}\new):
(1) \textbf{cross-validation methods} paritition the data under study into a
training (\data) and testing (\data{}\new) dataset,
whereas
(2) \textbf{posterior-preditive methods} generate \data{}\new via simulation.
}
...
Once a summary statistic is choosen, perhaps the simplest way to evaluate the
...
Similiar to LPML, the GG method preferred the model with most subsets (12;
partitioned by gene and codon),
in contrast to the marginal likelihood estimates, which favored the model
paritioned by coden (three subsets).
