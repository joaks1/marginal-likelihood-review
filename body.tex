...
Lartillot and Philippe (\citeyear{Lartillot2006}) introduced path sampling (PS;
...
(GHM) estimator that uses an arbitrary normalized reference distribution, as
...
The inflated-density ratio (IDR) estimator solves the problem of choosing a
...
Importantly, this work also revealed that it is important to model pattern
heterogeneity across sites of an alignment.
...
Baele et al.\ (\citeyear{Baele2012}) demonstrated that modelling among-branch
rate variation with a lognormal distribution tends to explain mammalian sequence
alignments better than using an exponential distribution.
...
likelihoods across a series of power-posterior distributions.
...
their performance on rich models and large datasets is still untested.

\ldots

Need some text on where to go from here:
More work on reference distributions that are as similar to the posterior
as possible, but easy to formulate and use.
This would advance GSS and GHM approaches.
For promising methods that work with only a sample from the posterior, like
IDR, making them easier to apply to phylogenetic models needs to be done.
