...
assumptions about the \vmdel{rate} \vmadd{probability} of the coin landing heads up
...
are analytically intractable.\kaccomment{Would it be appropriate or possible to briefly explain why this is intractable here? Or cite a good resource for understanding? Possibly explain a simple example?}
...
Lartillot and Philippe, \citeyear{Lartillot2006} for full details),
we get the ratio of the normalizing constants when $\beta$ equals 1 and 0, and
since we know the constant is 1 when $\beta$ is zero, we are left with the
marginal likelihood.}
...
% \vmcomment{Fix Maturana Patricio reference, no arxiv info.}
...
\citep{Jones2015}.
\vmadd{In fact, the first application of Bayes factors for phylogenetic model comparison was performed by \citet{Suchard2001} via reversible-jump MCMC. 
...
Fan et al.\ (\citeyear{Fan2011}) showed that, again, the HM estimator strongly
...
Baele et al.\ (\citeyear{Baele2012}) demonstrated that modelling
